 ...
 *
 * If count exceeds the collection size, returns all elements shuffled.
 *
 * @param count the number of elements to return
 * @return a list of random elements (may contain duplicates if count <= size)
 */
fun <T> Collection<T>.randomElements(count: Int): List<T> {
    if (isEmpty() || count <= 0) return emptyList()
    val effectiveCount = count.coerceAtMost(size)
    return shuffled().take(effectiveCount)
}
