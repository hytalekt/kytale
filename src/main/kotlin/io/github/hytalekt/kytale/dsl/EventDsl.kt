 ...
 *
 * This abstraction allows for easier testing by decoupling event registration
 * from the concrete JavaPlugin implementation.
 */
interface EventRegistrar {
     ...
     * @param handler the consumer to invoke when events fire
     */
    fun <T : IBaseEvent<*>> registerEvent(eventClass: Class<T>, handler: Consumer<T>)
}

/**
 * Creates an EventRegistrar backed by this JavaPlugin's event registry.
 */
fun JavaPlugin.asEventRegistrar(): EventRegistrar = object : EventRegistrar {
    @Suppress("UNCHECKED_CAST")
    override fun <T : IBaseEvent<*>> registerEvent(eventClass: Class<T>, handler: Consumer<T>) {
        val typedClass = eventClass as Class<IBaseEvent<Any>>
        val typedHandler = Consumer<IBaseEvent<Any>> { event -> handler.accept(event as T) }
        eventRegistry.registerGlobal(typedClass, typedHandler)
    }
}
...
inline fun JavaPlugin.events(block: EventRegistration.() -> Unit) {
    EventRegistration(this.asEventRegistrar()).apply(block)
}

/**
 * Builder class for registering multiple events.
 *
 * @property registrar the event registrar to register events with
 */
class EventRegistration(
    @PublishedApi internal val registrar: EventRegistrar
) {
    /**
     * Secondary constructor for JavaPlugin compatibility.
     */
    constructor(plugin: JavaPlugin) : this(plugin.asEventRegistrar())
    ...
    inline fun <reified T : IBaseEvent<*>> on(noinline handler: (T) -> Unit) {
        registrar.registerEvent(T::class.java, Consumer { handler(it) })
    }
    ...
    inline fun <reified T : IBaseEvent<*>> on(
        ...
        registrar.registerEvent(T::class.java, Consumer { event ->
            if (filter(event)) {
                handler(event)
            }
        })
