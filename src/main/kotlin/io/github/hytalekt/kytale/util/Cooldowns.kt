...
class Cooldown<K : Any>(
     ...
     *
     * @param key the key to check
     * @return the remaining duration, or null if not on cooldown
     */
    fun remaining(key: K): Duration? {
        val mark = cooldowns[key] ?: return null
        val elapsed = mark.elapsedNow()
        return if (elapsed >= duration) null else duration - elapsed
    }
